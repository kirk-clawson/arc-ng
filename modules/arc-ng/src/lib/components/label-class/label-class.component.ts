...
export class LabelClassComponent extends EsriAsyncComponentBase<__esri.LabelClass> {
  @Input()
  ...
  @ContentChild(TextSymbolDirective) set textChild(value: TextSymbolDirective) {
    this.temp = value;
    const textInstance = value.createInstance();
    console.log('Text Instance created', textInstance);
    this.setAutoCastField('symbol', textInstance);
  }
  ...
  private temp: TextSymbolDirective;
  ...
  async createInstance(): Promise<__esri.LabelClass> {
    ...
    params.symbol = this.temp.createInstance();
    // }
